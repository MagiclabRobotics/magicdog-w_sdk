...
constexpr uint8_t kWheelJointNum = 4;  ///< Number of wheel joints
...
enum class GaitMode : int32_t {
  ...
  GAIT_STAND_R = 2,          // Position control standing, RecoveryStand
  GAIT_LOWLEVL_SDK = 30,     // Low-level SDK gait
  GAIT_RL_MOVE_QUICK = 111,  // Fast movement
  GAIT_RL_TERRAIN = 112,     // All terrain
  GAIT_RL_CLIMB = 113,       // Climb stairs
  GAIT_RL_HAND_STAND = 114,  // Handstand
  GAIT_RL_FOOT_STAND = 115,  // Footstand
...
/**
 * @brief Single wheel joint control command
 */
struct SingleWheelJointCommand {
  double q_des = 0.0;    // desired joint position
  double dq_des = 0.0;   // desired joint velocity
  double tau_des = 0.0;  // desired feed-forward torque
...
struct LegJointCommand {
  ...
  std::array<SingleWheelJointCommand, kWheelJointNum> wheel_cmd;  ///< Wheel command array, in order of left and right
};
...
/**
 * @brief Single wheel joint state
 */
struct SingleWheelJointState {
  double q = 0.0;
  double dq = 0.0;
  double tau_est = 0.0;
};
...
struct LegState {
  ...
  std::array<SingleWheelJointState, kWheelJointNum> wheel_state;  ///< All wheel joint states
};
