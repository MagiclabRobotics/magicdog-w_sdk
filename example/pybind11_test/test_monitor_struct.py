...
# -*- coding: utf-8 -*-
"""
MagicDog_W SDK Python 使用示例 - 监控与状态结构体测试

本文件展示了如何使用 MagicDog_W SDK 的 Python 绑定来测试机器人监控相关的结构体和枚举。
...
print("=== 测试错误码枚举 ===")
error_code_ok = magicdog_w.ErrorCode.OK
error_code_timeout = magicdog_w.ErrorCode.TIMEOUT
print(f"错误代码(OK): {error_code_ok}")
print(f"错误代码(TIMEOUT): {error_code_timeout}")

# 测试错误码枚举值
if error_code_ok != magicdog_w.ErrorCode.OK:
    print(f"错误: OK错误码枚举值不匹配")
    sys.exit(1)
if error_code_timeout != magicdog_w.ErrorCode.TIMEOUT:
    print(f"错误: TIMEOUT错误码枚举值不匹配")
    sys.exit(1)
print("✓ 错误码枚举测试通过")

print("\n=== 测试状态结构体 ===")
status = magicdog_w.Status()
status.code = error_code_ok
status.message = "系统运行正常"
print(f"状态: 代码={status.code}, 信息='{status.message}'")

status_error = magicdog_w.Status()
...
if fault.error_message != "通信超时":
    ...
    sys.exit(1)
if fault2.error_code != magicdog_w.ErrorCode.INTERNAL_ERROR:
...
print("\n=== 测试电池状态与电源状态枚举 ===")
battery_state = magicdog_w.BatteryState.GOOD
battery_state_overheat = magicdog_w.BatteryState.OVERHEAT
print(f"电池状态(GOOD): {battery_state}")
print(f"电池状态(OVERHEAT): {battery_state_overheat}")

power_supply_status = magicdog_w.PowerSupplyStatus.CHARGING
power_supply_status_full = magicdog_w.PowerSupplyStatus.FULL
print(f"电池充放电状态(CHARGING): {power_supply_status}")
print(f"电池充放电状态(FULL): {power_supply_status_full}")

# 测试电池状态枚举值
if battery_state != magicdog_w.BatteryState.GOOD:
    print(f"错误: 电池状态GOOD枚举值不匹配")
    sys.exit(1)
if battery_state_overheat != magicdog_w.BatteryState.OVERHEAT:
    print(f"错误: 电池状态OVERHEAT枚举值不匹配")
    sys.exit(1)
if power_supply_status != magicdog_w.PowerSupplyStatus.CHARGING:
    print(f"错误: 电源状态CHARGING枚举值不匹配")
    sys.exit(1)
if power_supply_status_full != magicdog_w.PowerSupplyStatus.FULL:
    print(f"错误: 电源状态FULL枚举值不匹配")
    sys.exit(1)
print("✓ 电池状态与电源状态枚举测试通过")

print("\n=== 测试电池管理系统数据结构体 ===")
bms_data = magicdog_w.BmsData()
...
print("\n=== 测试机器人整体状态结构体 ===")
robot_state = magicdog_w.RobotState()
robot_state.faults = magicdog_w.FaultVector()
