...
# -*- coding: utf-8 -*-
"""
MagicDog_W SDK Python 使用示例 - 传感器结构体丰富示例

本文件展示了如何使用 MagicDog_W SDK 的 Python 绑定来测试机器人各类传感器相关的结构体和枚举。
...
double4_array = magicdog_w.Double4Array()
for i in range(4):
    double4_array[i] = i + 1.0
print(f"双精度四元数数组: {double4_array}")

double3_array = magicdog_w.Double3Array()
for i in range(3):
...
# ========== PointField 结构体 ==========
point_field = magicdog_w.PointField()
point_field.name = "x"
...
# ========== PointCloud2 结构体 ==========
point_cloud2 = magicdog_w.PointCloud2()
...
double_vector = magicdog_w.DoubleVector()
for i in range(3):
...
double9_array = magicdog_w.Double9Array()
for i in range(9):
    double9_array[i] = i + 1.0
print(f"Double9Array: {double9_array}")

double12_array = magicdog_w.Double12Array()
for i in range(12):
...
# ========== CameraInfo 结构体 ==========
camera_info = magicdog_w.CameraInfo()
...
trinocular_camera_frame = magicdog_w.TrinocularCameraFrame()
...
# ========== CompressedImage 结构体 ==========
compressed_image = magicdog_w.CompressedImage()
...
# ========== LaserScan 结构体 ==========
laser_scan = magicdog_w.LaserScan()
...
float_multi_array = magicdog_w.Float32MultiArray()
float_multi_array.layout.dim_size = 2
dim1 = magicdog_w.MultiArrayDimension()
dim1.label = "height"
dim1.size = 3
dim1.stride = 6
dim2 = magicdog_w.MultiArrayDimension()
dim2.label = "width"
dim2.size = 2
dim2.stride = 2
...
# ========== ByteMultiArray 结构体 ==========
byte_multi_array = magicdog_w.ByteMultiArray()
byte_multi_array.layout.dim_size = 1
dim = magicdog_w.MultiArrayDimension()
dim.label = "length"
dim.size = 10
dim.stride = 10
