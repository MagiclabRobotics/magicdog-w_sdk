...
> For more user guide, please go to [Qucik Start](https://support.magiclab.top/docs/dog_w/quick_start_reference)
...
To build your own application with this SDK, you can install the magicdog_w_sdk to specified directory:
```
  mkdir build
  cd build
  ...
  sudo make install
```
You can refer to example/cmake_sample on how to import the `magicdog_w_sdk` into your CMake project.
```
  cd example/cmake_example
  mkdir build
  cd build
  cmake .. -DCMAKE_PREFIX_PATH=/opt/magic_robotics/magicdog_w_sdk
  make -j8
```
